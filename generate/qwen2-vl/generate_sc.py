...
def generate_responses(model, processor, data_path, width, self_correction=False):
    """
    width must be 1 if self_correction is True
    """
    ...
    for datum in tqdm(data, desc="Inferencing"):
        ...
        if width != 1:
            ...
            if self_correction==True:
                init_response = response[0]

                datum["init_response"] = init_response

                messages.append({
                    "role": "system",
                    "content": [{"type": "text", "text": init_response}],
                })

                cor_prompt = f"""Review your previous answer and ensure that 
                ...
                messages.append({
                    "role": "user",
                    "content": [{"type": "text", "text": cor_prompt},],
                })
...
def main():
    ...
    output_path = f"output/qwen2vl-sc.json"

    model, processor = load_model(model_path)

    output = generate_responses(model, 
                                processor, 
                                data_path, 
                                width, 
